...
class test_snake(unittest.TestCase):        
    ...
    def test_wall_color(self):
        ...
        # Integer value
        color_int = 0xFFFFFFFF # (255, 255, 255, 255) -> white
        wall_color = Snake(self._sprite_size, 'darkgreen', 'green', 'red', color_int).wall_color
        self.assertIsInstance(wall_color, pygame.color.Color)
        self.assertEqual(wall_color, pygame.color.Color(color_int))

        # (r, g, b, [a])
        color_rgb = (255, 0, 0) # red
        wall_color = Snake(self._sprite_size, 'darkgreen', 'green', 'red', color_rgb).wall_color
        self.assertIsInstance(wall_color, pygame.color.Color)
        self.assertEqual(wall_color, pygame.color.Color(color_rgb))

        # Named color
        color_str = 'green'
        wall_color = Snake(self._sprite_size, 'darkgreen', 'green', 'red', color_str).wall_color
        self.assertIsInstance(wall_color, pygame.color.Color)
    ...
    def test_direction_without_save_update(self):
        ...
        snake = Snake(self._sprite_size, 'darkgreen', 'green', 'red', 'dimgrey', save_update = False)
    ...
    def test_direction_with_save_update(self):
        ...
        snake = Snake(self._sprite_size, 'darkgreen', 'green', 'red', 'dimgrey', save_update = True)
    ...
    def test_reset(self):
        """Test 'reset' of snake"""
        snake = Snake(self._sprite_size, 'darkgreen', 'green', 'red', 'dimgrey', 10, 0.5, save_update = True)        
        # Change direction
        snake.direction = pygame.K_UP
        snake.direction = pygame.K_LEFT        
        # Reset
        ...
        # Test
        for ii in range (0,5):
            self.assertEqual(snake.snake[ii], (int(self._screen_size[0]/4 + ii*self._sprite_size[0]), int(self._screen_size[1]/2)))
        self.assertEqual(snake.direction, pygame.K_RIGHT)
    ...
    def test_move(self):
        ...
        for direction in {pygame.K_UP, pygame.K_DOWN, pygame.K_LEFT, pygame.K_RIGHT}:            
            ...
            if direction == pygame.K_RIGHT:
                 self.assertEqual(new_snake.snake[-1][0], snake.snake[-1][0] + self._sprite_size[0])
                 self.assertEqual(new_snake.snake[-1][1], snake.snake[-1][1])
            elif direction == pygame.K_UP:
                self.assertEqual(new_snake.snake[-1][0], snake.snake[-1][0])
                self.assertEqual(new_snake.snake[-1][1], snake.snake[-1][1] - self._sprite_size[1])
