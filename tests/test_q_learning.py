...
class test_q_learning(unittest.TestCase):        
    def setUp(self):
        ...
        self._snake = Snake(self._sprite_size, Color.HEAD.value, Color.BODY.value, Color.APPLE.value, Color.WALL.value)
    ...
    def test_update(self):        
        env = SnakeQLearning(self._apple, self._snake, 0.0, 0.2, 0.5, 1000)        
    ...
    def test_get_reward_with_location_offset(self):
        ...
        for action_idx in range(0,4):
            self.assertEqual(env._get_reward_with_location_offset(512, action_idx, pygame.K_DOWN, pygame.K_RIGHT, 0), -10)
            self.assertEqual(env._get_reward_with_location_offset(256, action_idx, pygame.K_DOWN, pygame.K_RIGHT, 0), 10)
        self.assertEqual(env._get_reward_with_location_offset(128, 0, pygame.K_DOWN, pygame.K_RIGHT, 0), 1)
        self.assertEqual(env._get_reward_with_location_offset(0, 3, pygame.K_DOWN, pygame.K_RIGHT, 0), -1)
    ...
    def test_action(self):
        ...
        env = SnakeQLearning(self._apple, self._snake, 100.0, 0.2, 0.5, 1000)
        self.assertEqual(env._action(), pygame.K_RIGHT)
    
    def test_call_agent_for_action(self):
        env = SnakeQLearning(self._apple, self._snake, 0.2, 0.2, 0.5, 1000)
        ...
        for color in Color:
            ...
            match color:
                case Color.WALL:
                    ...
                    self.assertEqual(env._get_state_idx_with_offset(-self._sprite_size[0], 0, 4),32)
                case Color.BODY:
                ...
                case _:
                    ...
                    # Snake body to the left
                    self.assertEqual(env._get_state_idx_with_offset(-self._sprite_size[0], 0, 6),128)
